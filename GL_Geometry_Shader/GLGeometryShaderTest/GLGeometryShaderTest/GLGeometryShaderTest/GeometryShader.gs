...
void arrowPyramid4Dir(vec4 position, vec3 direction)
{    
	//direction
	vec3 transform_y = cross(vec3(0.0, 0.0, 1.0), normalize(direction));
	vec3 transform_z = normalize(direction);
	...
	transform = modelView*modelProj*transform;
...
void main() {
	...
	arrowPyramid4Dir(gl_in[0].gl_Position, direction[0]);
